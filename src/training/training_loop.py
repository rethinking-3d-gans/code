...
from src.training.embedders import TorchScriptEmbedder
...
def training_loop(
    ...
    common_kwargs = dict(c_dim=cfg.dataset.c_dim)
    D_img_resolution = loss_kwargs.cfg.training.patch.resolution if loss_kwargs.cfg.training.patch.strategy in ['normal', 'concat'] else training_set.resolution
    if loss_kwargs.cfg.training.mvc.adversarial:
        D_img_channels = training_set.num_channels * 2
    elif loss_kwargs.cfg.training.patch.strategy == 'concat':
        D_img_channels = training_set.num_channels * 2
    else:
        D_img_channels = training_set.num_channels
    G = dnnlib.util.construct_class_by_name(img_resolution=training_set.resolution, img_channels=training_set.num_channels, **G_kwargs, **common_kwargs).train().requires_grad_(False).to(device) # subclass of torch.nn.Module
    D = dnnlib.util.construct_class_by_name(img_resolution=D_img_resolution, img_channels=D_img_channels, **D_kwargs, **common_kwargs).train().requires_grad_(False).to(device) # subclass of torch.nn.Module
    ...
    if rank == 0:
        with torch.no_grad():
            ...
            G_kwargs = {'concat_lowres': loss_kwargs.cfg.training.mvc.adversarial} if loss_kwargs.cfg.training.mvc.adversarial else {}
            G_kwargs['camera_angles'] = camera_angles
            img = misc.print_module_summary(G, [z, c], module_kwargs=G_kwargs)
            if loss_kwargs.cfg.training.patch.strategy in ['normal', 'concat']:
                img = img[:, :, :loss_kwargs.cfg.training.patch.resolution, :loss_kwargs.cfg.training.patch.resolution] # [batch_size, c, patch_h, patch_w]
                if loss_kwargs.cfg.training.patch.strategy == 'concat':
                    img = torch.cat([img, img], dim=1) # [batch_size, 2 * c, patch_h, patch_w]
    ...
    if (cfg.training.mvc.perceptual_weight > 0.0 or cfg.training.Grec_coef > 0.0) and not cfg.training.perceptual_embedder in ['mse', 'mae']:
        embedder = TorchScriptEmbedder(cfg.training.perceptual_embedder, device).eval().requires_grad_(False).to(device)
    else:
        embedder = None
    ...
    loss = dnnlib.util.construct_class_by_name(device=device, G=G, D=D, embedder=embedder, augment_pipe=augment_pipe, **loss_kwargs) # subclass of training.loss.Loss
    phases = []

    # TODO: it makes sense to separate G and D phases construction?
    for name, module, opt_kwargs, reg_interval, rec_interval in [('G', G, G_opt_kwargs, G_reg_interval, cfg.training.Grec_interval), ('D', D, D_opt_kwargs, D_reg_interval, None)]:
        if reg_interval in [None, 0] and rec_interval in [None, 1] and not cfg.training.Grec_only:
            opt = dnnlib.util.construct_class_by_name(params=module.parameters(), **opt_kwargs) # subclass of torch.optim.Optimizer
            phases += [dnnlib.EasyDict(name=name+'all', module=module, opt=opt, interval=1)]
        else: # Lazy regularization.
            interval = min(reg_interval or float('inf'), rec_interval or float('inf'))
            mb_ratio = interval / (interval + 1)
            opt_kwargs = dnnlib.EasyDict(opt_kwargs)
            opt_kwargs.lr = opt_kwargs.lr * mb_ratio
            opt_kwargs.betas = [beta ** mb_ratio for beta in opt_kwargs.betas]
            opt = dnnlib.util.construct_class_by_name(module.parameters(), **opt_kwargs) # subclass of torch.optim.Optimizer
            phases += [dnnlib.EasyDict(name=name+'main', module=module, opt=opt, interval=1)]
            if not reg_interval is None:
                if name == 'G':
                    assert cfg.model.loss_kwargs.pl_weight > 0
                    phases += [dnnlib.EasyDict(name='Greg_pl', module=module, opt=opt, interval=interval)]
                else:
                    phases += [dnnlib.EasyDict(name='Dreg', module=module, opt=opt, interval=interval)]
            if not rec_interval is None:
                phases += [dnnlib.EasyDict(name=name+'rec', module=module, opt=opt, interval=rec_interval)]
    if cfg.training.Grec_only:
        phases = [p for p in phases if p.name == 'Grec']
    for phase in phases:
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            phase_real_img, phase_real_c, phase_real_camera_angles, phase_real_instance_id, phase_real_struct = batch['image'], batch['label'], batch['camera_angles'], batch['instance_id'], batch['struct']
            ...
            phase_real_instance_id = phase_real_instance_id.to(device).split(batch_gpu) # (batch_size // batch_gpu, [batch_gpu, h, w])
            phase_real_struct = phase_real_struct.to(device).split(batch_gpu) # (batch_size // batch_gpu, [batch_gpu, h, w])
            ...
            # PGC camera conditioning for G
            # Shift the values in 50% of random places by 1 to spoof the generator
            # Does not work for some reason :|
            # TODO: maybe spoofing is bad?
            all_gen_camera_angles_cond = all_gen_camera_angles.clone() # [N, 3]
            camera_spoof_idx = torch.rand(all_gen_camera_angles_cond.shape[0]) < cfg.model.generator.camera_cond_spoof_p # [N]
            ...
            all_gen_struct = [training_set.get_struct(i) for i in gen_cond_sample_idx] # [num_phases * batch_size, h, w]
            all_gen_struct = torch.from_numpy(np.stack(all_gen_struct)).pin_memory().to(device) # [num_phases * batch_size, h, w]
            all_gen_struct = [phase_gen_struct.split(batch_gpu) for phase_gen_struct in all_gen_struct.split(batch_size)] # [num_phases * batch_size, h, w]

        # Execute training phases.
        all_data = zip(phases, all_gen_z, all_gen_c, all_gen_camera_angles, all_gen_camera_angles_cond, all_gen_struct)
        for (phase, phase_gen_z, phase_gen_c, phase_gen_camera_angles, phase_gen_camera_angles_cond, phase_gen_struct) in all_data:
            ...
            for real_img, real_c, real_camera_angles, real_instance_id, real_struct, gen_z, gen_c, gen_camera_angles, gen_camera_angles_cond, gen_struct in phase_data:
                loss.accumulate_gradients(
                    phase=phase.name,
                    real_img=real_img,
                    real_c=real_c,
                    real_camera_angles=real_camera_angles,
                    real_instance_id=real_instance_id,
