...
from src.training.utils import linear_schedule, run_batchwise, extract_patches
...
class SynthesisBlocksSequence(torch.nn.Module):
    def __init__(self,
        ...
        last_block_fp32 = False,    # Force FP32 for the highest block
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for block_idx, res in enumerate(self.block_resolutions):
            ...
            use_fp16 = (res >= fp16_resolution) and not (last_block_fp32 and res == self.out_resolution)
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_seq_kwargs,     # Arguments of SynthesisBlocksSequence
        ...
        super().__init__()
        assert cfg.patch.strategy in [None, 'normal'], f"Unsupported patchwise training strategy: {cfg.patch.strategy}"
        self.cfg = cfg
        ...
        if self.cfg.backbone == 'stylegan2':
            self.tri_plane_decoder = SynthesisBlocksSequence(
                ...
                last_block_fp32=self.cfg.tri_plane.last_block_fp32,
        ...
        self.nerf_sp_beta = 1.0
        self.train_resolution = self.cfg.patch.resolution if self.cfg.patch.strategy in ['normal', 'concat'] else self.img_resolution
    ...
    def progressive_update(self, cur_kimg: float):
        ...
        self.nerf_sp_beta = linear_schedule(cur_kimg, self.cfg.nerf_sp_beta_init, self.cfg.nerf_sp_beta_target, self.cfg.nerf_sp_beta_kimg_growth)
    ...
    def render_spherical_bg(self, ws_bg: torch.Tensor, ray_o_world: torch.Tensor, ray_d_world: torch.Tensor, max_batch_res: int) -> torch.Tensor:
        ...
        bg_int_out: Dict = run_batchwise(
            ...
            clamp_mode=self.cfg.clamp_mode, noise_std=0.0, sp_beta=self.nerf_sp_beta)
    ...
    def forward(self, ws, camera_angles: torch.Tensor, patch_params: Dict=None, max_batch_res: int=128, return_depth: bool=False, ignore_bg: bool=False, bg_only: bool=False, fov=None, **block_kwargs):
        ...
        if self.cfg.bg_model.type == "plane":
            ...
            if not patch_params is None:
                bg = extract_patches(bg, patch_params, resolution=self.cfg.patch.resolution, strategy=self.cfg.patch.strategy) # [batch_size, c, h_patch, w_patch]
        ...
        weights = run_batchwise(
            ...
            batch_size=max_batch_res ** 2,
            dim=1,
            clamp_mode=self.cfg.clamp_mode, noise_std=nerf_noise_std, sp_beta=self.nerf_sp_beta, use_inf_depth=self.cfg.bg_model.type is None,
        ...
        int_out: Dict = run_batchwise(
            ...
            noise_std=nerf_noise_std, sp_beta=self.nerf_sp_beta, use_inf_depth=self.cfg.bg_model.type is None)
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, camera_angles, camera_angles_cond=None, instance_id=None, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ws = self.mapping(z, c, camera_angles=camera_angles_cond, instance_id=instance_id, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
