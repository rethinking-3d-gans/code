...
def launch_training(c, outdir, dry_run):
    ...
    print(f'Dataset num instances:   {c.G_kwargs.mapping_kwargs.num_instances}')
    print()
...
def init_dataset(cfg: DictConfig):
    try:
        ...
        return dataset_kwargs, dataset.compute_num_instances(), mean_camera_pose
...
def main(cfg: DictConfig):
    ...
    # Training set.
    c.training_set_kwargs, num_instances, mean_camera_pose = init_dataset(cfg)
    ...
    c.training_set_kwargs.use_instances = opts.use_instances
    ...
    c.G_kwargs.mapping_kwargs.num_instances = num_instances if opts.use_instances else 0
    ...
    elif cfg.model.name == 'pigan':
        c.G_kwargs.class_name = 'training.networks_pigan.Generator'
    ...
    if opts.use_instances or cfg.training.Grec_coef > 0.0:
        assert num_instances > 0, "You need to label instances in the dataset to enable the reconstruction loss."
    ...
    if cfg.training.patch.strategy in ['normal', 'concat']:
        ...
        if cfg.training.patch.distribution == 'categorical':
            assert sum(cfg.training.patch.probs) == 1.0, f"Probabilities do not sum to 1.0: {cfg.training.patch.probs}"
            assert min(cfg.training.patch.support) * cfg.dataset.resolution == cfg.training.patch.resolution, \
                f"Minimal patch resolution does not correspond to data distribution: {cfg.training.patch}, {cfg.dataset.resolution}"
