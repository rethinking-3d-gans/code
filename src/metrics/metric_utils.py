...
class MetricOptions:
    def __init__(self, G=None, G_kwargs={}, dataset_kwargs={}, num_gpus=1, rank=0, device=None, progress=None, cache=True, batch_gen=None):
        ...
        self.cache          = cache
        self.batch_gen      = batch_gen # Generator batch size
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, **stats_kwargs):
    batch_gen = min(batch_size, 4) if opts.batch_gen is None else opts.batch_gen
    assert batch_size % batch_gen == 0

    # Setup generator and labels.
    G = copy.deepcopy(opts.G).eval().requires_grad_(False).to(opts.device)
    cond_iter = iterate_random_conditioning(opts=opts, batch_size=batch_gen)
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            c, camera_angles = next(cond_iter)
            img = G(z=z, c=c, camera_angles=camera_angles, **opts.G_kwargs)
